...
def arg_mapping: { #! Do not Change this used for Demo only... As this will change .args! will be fatal
  ...
  def get_keys: #? Functions to Convert modmask into Keys, There should be a better math for this but Im lazy
    if . == 0 then
      ""
    elif . >= 64 then
...
#? will display on the terminal if wofi is not found or have -j flag
...
selected=$(echo  "$output" | wofi -dmenu -p "" -theme-str "${fnt_override}" -theme-str "${r_override}" -theme-str "${icon_override}" -config "${roconf}" | sed 's/.*\s*//')
...
#? If flag is repeat then repeat wofi if not then just execute once
