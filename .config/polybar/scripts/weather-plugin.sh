...
COLOR_CLOUD="#eff1f5"
COLOR_THUNDER="#d9822b"
COLOR_LIGHT_RAIN="#50a9e6"
COLOR_HEAVY_RAIN="#b3deef"
COLOR_SNOW="#FFFFFF"
COLOR_FOG="#363c6e"
COLOR_TORNADO="#d9822b"
COLOR_SUN="#f5b142"
COLOR_MOON="#FFFFFF"
COLOR_ERR="#eb5368"
COLOR_WIND="#50a9e6"
...
function setIcons {
    ...
    if [ "$DISPLAY_WIND" = "yes" ] && [ `echo "$WINDFORCE >= $MIN_WIND" |bc -l` -eq 1 ]; then
        WIND="%{T$WEATHER_FONT_CODE}%{F$COLOR_WIND}$WINDICON%{F-}"
    ...
    if [ "$TEMP" -le $COLD_TEMP ]; then
        TEMP="%{F$COLOR_COLD}%{T$TEMP_FONT_CODE}%{F-} $COLOR_TEXT_BEGIN$TEMP%{T$TEMP_FONT_CODE}$TEMP_ICON$COLOR_TEXT_END"
...
function outputCompact {
    OUTPUT="$WIND %{T$WEATHER_FONT_CODE}%{F$ICON_COLOR}$ICON%{F-}$ERR_MSG$COLOR_TEXT_BEGIN $DESCRIPTION$COLOR_TEXT_END| $TEMP"
    echo "$OUTPUT"
}
